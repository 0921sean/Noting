...
// ─── 카테고리 메타데이터 ───────────────────────────────────────────────────────
const _categoryMeta = {
  'idea': (label: '아이디어', color: Color(0xFFD4843A)),
  'todo': (label: '할 일', color: Color(0xFF7C5C3E)),
  'thought': (label: '생각', color: Color(0xFF8B8070)),
};
...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  bool get _hasCategoryData => _notes.any((n) => n.category != null);
  ...
  Widget _buildCategoryFilter() {
    const filters = [null, 'idea', 'todo', 'thought'];
    const labels = ['전체', '아이디어', '할 일', '생각'];
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: List.generate(filters.length, (i) {
          final selected = _selectedCategory == filters[i];
          final meta = filters[i] != null ? _categoryMeta[filters[i]] : null;
          final activeColor = meta?.color ?? Theme.of(context).colorScheme.primary;

          return GestureDetector(
            onTap: () => setState(() => _selectedCategory = filters[i]),
            child: AnimatedContainer(
  ...
  Widget _buildEmptyState() {
    ...
    if (_selectedCategory != null) {
      final label = _categoryMeta[_selectedCategory]?.label ?? '';
      return Center(
  ...
  Widget _buildCategoryChip(String category) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: meta.color.withOpacity(0.3), width: 0.5),
      ),
      child: Text(
        meta.label,
        style: TextStyle(
