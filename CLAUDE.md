...
> 전체 규칙은 [CONVENTION.md](CONVENTION.md). 핵심 = **추적성** — 모든 변경이 "어떤 목표에서 왔는지" 이슈로 이어지게.
> 작업 단위는 **Epic · Task · 단발 하이브리드**.

- **Epic** = 함께 가는 몇 개 작업의 묶음 → 이슈 `[Epic] #N` + **투두 체크리스트** (PR 없음).
- **Task** = Epic의 투두 하나 → **이슈 안 만듦**, 브랜치 `type/슬러그`, PR `[Type] #N - 내용`(에픽번호) + 본문 `Part of #N`.
- **단발** = Epic 밖 독립 작업 → 이슈 `[Type] #K`, 브랜치 `type/#K`, PR `[Type] #K - 내용` + 본문 `Closes #K`(자동종료).
- **Type**: Epic/Feat/Fix/Hotfix/Refactor/Design/Setting/Docs/Test/Chore
- **커밋**: `[Type/#번호] 내용` (Task=에픽번호, 단발=자기번호). **Co-Authored-By 라인 없음**.
- `Closes`는 **단발 전용**. Task는 `Part of #N`만 — Epic은 체크리스트로 추적하다 마지막에 **수동 종료**.
- **의미 단위 커밋**: 기능1/버그1/리팩터1 = 커밋1. ❌ `update`·`wip`·`작업함`·빈 커밋 금지 (히스토리는 채용 때 평가됨). (2026-07-10 이전 커밋은 옛 포맷 — 소급 변경 안 함)
- **세션 시작**: `git fetch`로 origin 관계 확인 → **새 커밋 있을 때만** `pull`.
- **세션 끝**: 남은 변경 커밋 → `push`. 마지막 응답에 "이번 세션에 한 일" 1~2줄 요약.

### ⚠️ 절대 규칙 (라이브 안전장치)

1. **`main` 직접 커밋·푸시 금지.** 항상 브랜치 → PR → 셀프리뷰 → CI 통과 → 머지. (라이브 긴급 Hotfix만 예외, 사후 이슈화)
2. **머지는 절대 자동으로 하지 않는다.** CI가 초록이어도 `"CI 초록, 리뷰 대기"`에서 멈추고 사용자에게 넘긴다. **사용자가 리뷰하고 직접 머지한다.** 승인은 **PR별** — 한 PR의 OK가 다음으로 안 이어짐.
3. **PR 코멘트엔 프롬프트(채팅)에서 답하지 않고 PR에서 직접 답글**을 단다(팀원처럼). 질문이면 답글로, `P1 …` 같은 수정요청이면 **같은 브랜치에 커밋을 추가**하고 그 내용으로 답글.
4. **위험/라이브 변경은 기능 플래그 뒤에.** 기본 off로 머지 → 켜는 건 별도 승인(예: `FEATURE_X_ENABLED`).
5. **커밋 금지 대상**: `.db`·`.session`·`.env` 등 시크릿 — 커밋 전 `.gitignore` 확인.
